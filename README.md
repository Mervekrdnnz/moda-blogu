# 👗 Moda Bloğu (Moda-Blogu)

Bu proje, moda dünyasındaki son trendlerin, stil önerilerinin ve kişisel yazıların paylaşıldığı modern ve kullanıcı dostu bir blog platformudur.

## 🚀 Özellikler

- **Responsive Tasarım:** Mobil, tablet ve masaüstü cihazlarla tam uyumlu.
- **Dinamik İçerik:** (Eğer varsa: Yazı ekleme, düzenleme ve silme özellikleri).
- **Kategoriler:** Stil önerileri, sezon trendleri, ayakkabı & aksesuar gibi kategorilere ayrılmış içerikler.
- **Modern Arayüz:** Kullanıcı deneyimini ön planda tutan şık tasarım.

## 🛠️ Kullanılan Teknolojiler

Bu projeyi geliştirmek için aşağıdaki araçlar ve diller kullanılmıştır:

* **Frontend:** HTML5, CSS3, JavaScript 
* **Styling:** Bootstrap / Tailwind CSS / Sass
* **Backend (Varsa):** Node.js / Python / Firebase
* **Fontlar:** Google Fonts (Poppins, Montserrat vb.)

## 🏁 Başlarken

Projeyi yerel bilgisayarınızda çalıştırmak için şu adımları izleyin:

1.  **Depoyu kopyalayın:**
    ```bash
    git clone [https://github.com/Mervekrdnnz/moda-blogu.git](https://github.com/Mervekrdnnz/moda-blogu.git)
    ```
2.  **Proje dizinine gidin:**
    ```bash
    cd moda-blogu
    ```
3.  **İndeks dosyasını açın:**
    * Sadece HTML ise `index.html` dosyasını tarayıcıda açmanız yeterlidir.
    * (Eğer bir framework kullanıyorsan: `npm install` ve `npm start`)
